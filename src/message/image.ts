...
export class Image {
	...
	constructor(elem: ImageElem, private dm = false, private cachedir?: string) {
		...
		if (image instanceof Buffer) {
			this.task = this.fromProbeSync(image)
		} else if (typeof image !== "string") {
			throw new Error("bad file param: " + image)
		} else if (image.startsWith("base64://")) {
			this.task = this.fromProbeSync(Buffer.from(image.slice(9), "base64"))
		} else if (image.startsWith("http://") || image.startsWith("https://")) {
			this.task = this.fromWeb(image)
		} else {
			this.task = this.fromLocal(image)
		}
	}
